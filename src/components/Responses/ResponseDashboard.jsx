...
const ResponseDashboard = () => {
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      // We strip out any stray markdown ```json tags the AI might accidentally leave
      const cleanJsonStr = data.analysis.replace(/```json/gi, '').replace(/```/g, '').trim();
      
      // Parse the JSON directly! No more string hacking!
      const aiObject = JSON.parse(cleanJsonStr);
      setParsedData(aiObject);

    } catch (err) {
      console.error(err);
      setError("Failed to connect to AI or received invalid data format.");
    } finally {
      setLoadingAI(false);
    }
  };
        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          {!parsedData && !loadingAI && (
            <button onClick={analyzeWithAI} className="bg-indigo-600 text-white px-6 py-3 rounded-xl hover:bg-indigo-700 transition shadow-lg font-bold flex items-center gap-2">
              <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clipRule="evenodd" /></svg>
            ...
            {!parsedData && !loadingAI && !error && (
              <div className="bg-white rounded-2xl p-12 shadow-sm border border-gray-100 flex flex-col items-center justify-center flex-1 text-center">
                <div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mb-4">
                  <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="h-10 w-10 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
                </div>
                <h3 className="text-2xl font-bold text-gray-800 mb-2">Engine Ready</h3>
                <p className="text-gray-500 max-w-md">Click the button above to generate a full visual diagnostic report.</p>
              </div>
            )}
            ...
            {parsedData && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <div className="space-y-6">
                  ...
                  {parsedData.departments && parsedData.departments.length > 0 && (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-rose-50/50 p-6 rounded-2xl border border-rose-100">
                      <h3 className="text-xs font-bold text-rose-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                        Key Vulnerabilities
                      </h3>
                      <ul className="space-y-3">
                        {parsedData.risks && parsedData.risks.length > 0 ? parsedData.risks.map((risk, i) => (
                          <li key={i} className="text-gray-700 text-sm flex items-start gap-2">
                            <span className="text-rose-400 mt-0.5">•</span><span className="leading-relaxed">{risk}</span>
                          </li>
                    ...
                    <div className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100">
                      <h3 className="text-xs font-bold text-emerald-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                        Action Plan
                      </h3>
                      <ul className="space-y-3">
                        {parsedData.recommendations && parsedData.recommendations.length > 0 ? parsedData.recommendations.map((rec, i) => (
                          <li key={i} className="text-gray-700 text-sm flex items-start gap-2">
                            <span className="text-emerald-500 mt-0.5">→</span><span className="leading-relaxed">{rec}</span>
                          </li>
